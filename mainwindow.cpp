...
Requirement* req_list[3] = {nullptr};
Semester* sem_list[8] = {nullptr};
...
void MainWindow::init_back_end() const{
    ...
    for (int i = 1; i <= 8; i++) {
        if (i == 1) {
            Semester* new_semester = new Semester(i, nullptr);
            sem_list[i-1] = new_semester;
        }
        else {
            Semester* new_semester = new Semester(i, sem_list[i-2]);
            sem_list[i-1] = new_semester;
        }
    }
}
...
void MainWindow::add_course(string course_name, Semester* semester) const{
    semester -> add_course(course_name);
}
        ...
        for (int i = 0; i < section_list.size(); i++){
            ...
            for (int j = 0; j < course_name_list.size(); ++j) {
                ...
                if (course_name_list[j] == chosen_course) {
                    new_course_btn -> setStyleSheet("background-color: rgb(180, 225, 255)");
                }
                //connection
                QObject::connect(new_course_btn, SIGNAL(is_clicked(string)), this, SLOT(user_click_a_course(string)));
            ...
            ui -> SectionLayout -> addWidget(line);
        }
    }
}
...
void MainWindow::display_semester(Semester* semester){
    ...
    //delete old content
    while(sem_layout_list[index] -> count() != 1){
    ...
    for (int i = 0; i < course_list.size(); ++i) {
        ...
        //connections
        QObject::connect(new_course_btn, SIGNAL(is_clicked(string,Semester*)), this, SLOT(user_delete_a_course(string,Semester*)));
        QObject::connect(new_course_btn, SIGNAL(is_viewed(Course*)), this, SLOT(user_view_a_course(Course*)));
        QObject::connect(new_course_btn, SIGNAL(is_left()), this, SLOT(user_leave_a_course()));
        //add to form
        sem_layout_list[index] -> insertWidget(sem_layout_list[index] -> count() - 1,new_course_btn);
    }
}
...
void MainWindow::on_choice_ccc_currentTextChanged(QString text){
    ...
    if (chosen_requirement != nullptr && chosen_requirement -> type == Requirement::Type::CCC) need_to_change = true;
...
void MainWindow::on_choice_school_currentTextChanged(QString text){
    bool need_to_change = false;
    if (chosen_requirement != nullptr && chosen_requirement -> type == Requirement::Type::SCHOOL) need_to_change = true;
...
void MainWindow::on_choice_major_currentTextChanged(QString text){
    bool need_to_change = false;
    if (chosen_requirement != nullptr && chosen_requirement -> type == Requirement::Type::MAJOR) need_to_change = true;
...
void MainWindow::on_txt_search_box_textChanged(QString text){
